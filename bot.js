...
bot.start(async (ctx) => {
    ...
    setTimeout(async () => {
        if (ctx.session.timeOut1) {
            ...
            try {
                await ctx.replyWithHTML(messages.prePaymentVideoAdvMessage.ru, keyboards.prePaymentVideoAdvMessageKeyboard)
            } catch (error) {
                console.error(error);
            }
        }
        ...
        const ratesDescriptionWithVideoMessageKeyboard = Markup.keyboard([
            Markup.button.webApp("Выбрать тариф🎁", `${webAppUrl}/choose-rate?userId=${userId}`)
        ]).resize().oneTime();
            ...
            if (ctx.session.timeOut2) {
                ...
                setTimeout(async () => {
                    if (ctx.session.timeOut4) {
                        try {
                            await ctx.replyWithHTML(messages.needHelpChoosingRateMessage.ru, ratesDescriptionWithVideoMessageKeyboard)
                        } catch (error) {
                            console.error(error);
                        }
                    }
...
bot.action("prePaymentVideoAdvMessage-authorInfoWithPicMessage", async (ctx) => {
    ...
    setTimeout(async () => {
        try {
            await ctx.replyWithHTML(messages.authorInfoWithoutPicMessage.ru, keyboards.authorInfoWithoutPicMessageKeyboard)
...
bot.on("web_app_data", async ctx => {
    ...
    if (data.webAppType === 'choose-rate') {
        ...
        try {
            await mainDb.updateUserParameter(userId, "userRate", ctx.session.userRate)
        } catch (error) {
            console.error(error);
            setTimeout(async () => {
                await mainDb.updateUserParameter(userId, "userRate", ctx.session.userRate)
        ...
        setTimeout(async () => {
            if (ctx.session.timeOut5) {
                await ctx.replyWithHTML(messages.paymentIssuesMessage.ru, paymentIssuesMessageKeyboard)
            }
        }, TenMinTimer);
        ...
        try {
            await ctx.replyWithPhoto({source: './assets/images/successPayment.jpg'}, {
                protect_content: true,
        ...
        const firstLessonTestStartKeyboard = Markup.keyboard([
            [Markup.button.webApp("Проверить знания📝", `${webAppUrl}/test-one?userId=${userId}`)]
        ]).resize().oneTime();
        ...
        try {
            await mainDb.updateUserAfterPaymentInfo(userId, data.userEmail, data.userPhone, data.userName, data.paymentPrice, data.productId)
        } catch (error) {
            console.error(error);
            setTimeout(async () => {
                await mainDb.updateUserAfterPaymentInfo(userId, data.userEmail, data.userPhone, data.userName, data.paymentPrice, data.productId)
        ...
        if (data.webAppType === 'test-one-passed') {
            ...
            try {
                await mainDb.updateUserTests(userId, "userFirstTestPassed", false, "userFirstTestSkipped", true, "userFirstTestAttempts", data.testAttempts)
            } catch (error) {
                console.error(error);
                setTimeout(async () => {
                    await mainDb.updateUserTests(userId, "userFirstTestPassed", false, "userFirstTestSkipped", true, "userFirstTestAttempts", data.testAttempts)
        ...
        try {
            await ctx.replyWithSticker("CAACAgIAAxkBAAEC-5tlrPIYP3qiZYDaeQrmNfABQJOJCAAC_gADVp29CtoEYTAu-df_NAQ", {
                protect_content: true,
                reply_markup: { remove_keyboard: true }
            })
        } catch (error) {
        ...
        if (data.webAppType === 'test-two-passed') {
            ...
            try {
                await mainDb.updateUserTests(userId, "userSecondTestPassed", false, "userSecondTestSkipped", true, "userSecondTestAttempts", data.testAttempts)
            } catch (error) {
                console.error(error);
                setTimeout(async () => {
                    await mainDb.updateUserTests(userId, "userFirstTestPassed", false, "userFirstTestSkipped", true, "userFirstTestAttempts", data.testAttempts)
        ...
        const thirdLessonProAdvancedIntroKeyboard = Markup.inlineKeyboard([
            [Markup.button.url('Смотреть видео🖥', `${webAppUrl}/${route[ctx.session.userRate]}`)]
        ]);
        ...
        setTimeout(async () => {
            await ctx.replyWithPhoto({source: './assets/images/lesson3.jpg'}, {
                protect_content: true,
                parse_mode: 'HTML',
                caption: ctx.session.userRate === 'basic' ? messages.thirdLessonBasicIntroMessage.ru : messages.thirdLessonProAdvancedIntroMessage.ru,
        ...
        const currentTime = await functions.getCurrentTime()
        try {
            await mainDb.updateUserPersonalAnswersInfo(userId, data.userAnswName, data.userAnswInst, data.userAnswWhoAreYou, data.userAnswAim, data.userAnswAimRealize, data.userAnswWeaknesses, data.userAnswClient, currentTime)
        } catch (error) {
            console.error(error);
            setTimeout(async () => {
                await mainDb.updateUserPersonalAnswersInfo(userId, data.userAnswName, data.userAnswInst, data.userAnswWhoAreYou, data.userAnswAim, data.userAnswAimRealize, data.userAnswWeaknesses, data.userAnswClient, currentTime)
        ...
        if (String(data.productId) === '27004') {
            ...
            try {
                await ctx.replyWithHTML(messages.advancedUpgradeSuccessPaymentMessage.ru, signUpForSessionIntroKeyboard)
            } catch (error) {
               console.error(error);
               await ctx.replyWithHTML(messages.techProblemsMessage.ru) 
            }
        ...
        if (String(data.productId) === '27008') {
            ...
            try {
                await ctx.replyWithHTML(messages.basicUpgradeToAdvancedLessonFourIntroMessage.ru, fourthLessonBasicIntroKeyboard)
            } catch (error) {
                console.error(error);
                await ctx.replyWithHTML(messages.techProblemsMessage.ru)
            }
        ...
        if (String(data.productId) === '27010') {
            ...
            try {
                await ctx.replyWithHTML(messages.basicUpgradeToProLessonFourIntroMessage.ru, fourthLessonBasicIntroKeyboard)
            } catch (error) {
                console.error(error);
                await ctx.replyWithHTML(messages.techProblemsMessage.ru)
            }
            ...
            setTimeout(async () => {
                ...
                try {
                    await ctx.replyWithHTML(messages.signUpForSessionIntroMessage.ru, signUpForSessionIntroKeyboard)
                } catch (error) {
                    console.error(error);
                    await ctx.replyWithHTML(messages.techProblemsMessage.ru)
                }
...
bot.hears("Задать вопрос❓", async (ctx) => {
    ...
    try {
        await ctx.replyWithHTML(messages.needHelpMessage.ru, paymentLinkMessageKeyboard)
    } catch (error) {
        console.error(error);
        await ctx.replyWithHTML(messages.techProblemsMessage.ru)
    }
...
bot.hears("ТЫ КТО чтоб так базарить?😡", async (ctx) => {
    ...
    setTimeout(async () => {
        try {
            await ctx.replyWithHTML(messages.authorInfoWithoutPicMessage.ru, keyboards.getBackToPaymentKeyboard)
        } catch (error) {
            console.error(error);
            await ctx.replyWithHTML(messages.techProblemsMessage.ru)
        }
        ...
        try {
            ...
            setTimeout(async () => {
                if (ctx.session.timeOut7) {
                    await ctx.replyWithHTML(messages.advancedUpgradeNoPaymentMessage.ru, advancedUpgradeOfferKeyboard)
                    setTimeout(async () => {
                        if (ctx.session.timeOut8) {
                            await ctx.replyWithHTML(messages.advancedUpgradeNoPaymentFinalMessage.ru, {
                                reply_markup: {remove_keyboard: true}
                            })
                        }
...
bot.action("basicUpgradeOfferMessage-basicUpgradeToAdvancedPaymentMessage", async (ctx) => {
    ...
    try {
        await ctx.replyWithHTML(messages.basicUpgradeToAdvancedPaymentLinkMessage.ru, basicToAdvancedUpgradeOfferKeyboard)
    } catch (error) {
        console.error(error);
        await ctx.replyWithHTML(messages.techProblemsMessage.ru)
    }
...
bot.action("basicUpgradeOfferMessage-basicUpgradeToProPaymentMessage", async (ctx) => {
    ...
    try {
        await ctx.replyWithHTML(messages.basicUpgradeToProPaymentLinkMessage.ru, basicToProUpgradeOfferKeyboard)
    } catch (error) {
        console.error(error);
        await ctx.replyWithHTML(messages.techProblemsMessage.ru)
    }
